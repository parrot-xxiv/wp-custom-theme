...
if ( ! defined( 'WP_CUSTOM_THEME_VERSION' ) ) {
	// Replace the version number of the theme on each release.
	define( 'WP_CUSTOM_THEME_VERSION', '1.0.0' );
}

/**
 * Sets up theme defaults and registers support for various WordPress features.
 *
 * Note that this function is hooked into the afterWP_CUSTOM_THEMEetup_theme hook, which
 * runs before the init hook. The init hook is too late for some features, such
 * as indicating support for post thumbnails.
 */
function custom_themeWP_CUSTOM_THEMEetup() {
	/*
	...
	add_themeWP_CUSTOM_THEMEupport( 'automatic-feed-links' );
	...
	/*
		...
		* provide it for us.
		*/
	add_themeWP_CUSTOM_THEMEupport( 'title-tag' );
...
function custom_theme_scripts() {
	wp_enqueue_style( 'custom-theme-style', get_stylesheet_uri(), array(), WP_CUSTOM_THEME_VERSION );
	wp_style_add_data( 'custom-theme-style', 'rtl', 'replace' );

	wp_enqueue_script( 'custom-theme-navigation', get_template_directory_uri() . '/js/navigation.js', array(), WP_CUSTOM_THEME_VERSION, true );
